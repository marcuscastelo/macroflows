...
export function createApiFoodImportService(deps: {
  ...
  async function importFoodsFromApiByName(name: string): Promise<Food[]> {
    ...
    if (apiFoods.length === 0) {
      localShowError(`Nenhum alimento encontrado para "${name}"`)
      return []
    }
    ...
    if (upsertionResults.some((result) => result.status === 'rejected')) {
      ...
      if (relevantErrors.length > 0) {
        ...
        await cachedSearchCrud.markSearchAsCached(name)
      }
    } else {
