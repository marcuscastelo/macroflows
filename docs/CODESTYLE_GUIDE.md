...
### Error Handling Pattern Migration (2025-12)

**Issue**: [#95 - Migrate critical application-layer error handling to logging + showError pattern](https://github.com/marcuscastelo/macroflows/issues/95)

**Summary**: This migration standardizes application-layer error handling across the codebase by ensuring:
1. `logging.error()` is called BEFORE `showError()` for observability
2. Structured context (component name, IDs) is included in logging calls
3. User-facing errors are displayed via `showError()` with appropriate context

**Pattern Order**:
```typescript
// CORRECT: logging first, then showError
try {
  await domainFunc()
} catch (error) {
  logging.error('ComponentName operation error:', error, {
    component: 'ComponentName',
    relevantId: someId,
  })
  showError(error, { context: 'user-action' }, 'Friendly message')
}
```

**Files Migrated**:
- `RecipeEditView.tsx` - Multiplier update handlers
- `DayMeals.tsx` - Meal and item update handlers
- `BodyMeasureView.tsx` - Measure delete handler
- `ImportExportModalContent.tsx` - Import/export error handlers
- `macroTargetUseCases.ts` - Missing data warnings
- `recipeItemUseCases.ts` - Fixed order of logging/showError
- `RestoreProfileModal.tsx` - Profile restore handler

**Reasoning**: Explicit logging at each application boundary provides better observability and debugging capabilities than a centralized error handler. This pattern aligns with Clean Architecture principles where the application layer is responsible for orchestration and error handling.
