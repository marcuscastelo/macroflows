# CLAUDE.md

> Doc status: pointer.
> This file is a Claude-specific pointer, not the source of truth.

Canonical repo policy lives in [AGENTS.md](./AGENTS.md).

If anything in this file conflicts with `AGENTS.md` or the canonical docs, the canonical docs win.

## Claude-specific notes

- Claude transport and permissions for this repo live in `.claude/settings.json`.
- Claude command assets remain under `.claude/commands/`.
- Use the canonical workflow and command defaults from `AGENTS.md`.
